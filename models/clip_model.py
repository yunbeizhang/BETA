...
class CLIPZeroShotClassifier(nn.Module):
    """
    A zero-shot classifier using OpenAI's CLIP ViT-B/16 model.
    Converts ImageNet class names to text embeddings using the template "a photo of a <CLS>."
    """
    
    def __init__(self, dataset_name: str = "imagenet", device: str = None):
        super().__init__()
    ...
    def _create_text_embeddings(self) -> torch.Tensor:
        """
        Create text embeddings for all class names using the template "a photo of a <CLS>."
        
        Returns:
            torch.Tensor: Normalized text embeddings of shape (num_classes, embedding_dim)
        """
        # Create text prompts using the template
